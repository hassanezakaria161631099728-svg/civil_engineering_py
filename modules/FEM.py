...
def create_vertical_X_bracing_elements(nf: int):
    """
    Create element connectivity for a vertical X-braced wall system.
    Returns
    -------
    elements : np.ndarray
        Array of element connectivity [[n1, n2], ...]
    """

    elements = []

    n_levels = nf + 1
    left_nodes = np.arange(0, n_levels)
    right_nodes = np.arange(n_levels, 2 * n_levels)

    # -------------------------------------------------
    # 1. Vertical elements (left & right chords)
    # -------------------------------------------------
    for i in range(nf):
        elements.append([left_nodes[i], left_nodes[i + 1]])
        elements.append([right_nodes[i], right_nodes[i + 1]])

    # -------------------------------------------------
    # 2. Horizontal elements (NO ground level)
    # -------------------------------------------------
    for i in range(1, n_levels):
        elements.append([left_nodes[i], right_nodes[i]])

    # -------------------------------------------------
    # 3. Diagonal X-bracing elements
    # -------------------------------------------------
    for i in range(nf):
        # Left bottom to right top
        elements.append([left_nodes[i], right_nodes[i + 1]])
        # Right bottom to left top
        elements.append([right_nodes[i], left_nodes[i + 1]])
...
def create_vertical_bracing_truss(nf: int, h: float, L: float):
    ...
    elements = create_vertical_X_bracing_elements(nf)
...
def plot_frame(nodes, elements, elem_props, loads, constraints, load_scale):
...
def generate_columns(nx, ny):
    columns = []
    eid = 0

    for i in range(nx + 1):        # left → right
        for j in range(ny):        # bottom → top
            n1 = j * (nx + 1) + i
            n2 = (j + 1) * (nx + 1) + i
            columns.append((eid, n1, n2, "column"))
            eid += 1

    return columns

def generate_beams(nx, ny, start_eid=0):
    beams = []
    eid = start_eid

    for j in range(1,ny + 1):        # bottom+1(ground level excluded) → top
        for i in range(nx):        # left → right
            n1 = j * (nx + 1) + i
            n2 = j * (nx + 1) + (i + 1)
            beams.append((eid, n1, n2, "beam"))
            eid += 1

    return beams

def generate_elements(nx, ny):
    elements = []

    columns = generate_columns(nx, ny)
    elements.extend(columns)

    beams = generate_beams(nx, ny, start_eid=len(columns))
...
def plot_grid_elevation_view_XZ(nodes, elements, nx, nz,folder,filename):
    ...
    for eid, n1, n2, etype in elements:
        x1, z1 = nodes[n1][1], nodes[n1][2]
        x2, z2 = nodes[n2][1], nodes[n2][2]

        if etype == "column":
            ax.plot([x1, x2], [z1, z2], color="yellow", linewidth=2)
        elif etype == "beam":
            ax.plot([x1, x2], [z1, z2], color="green", linewidth=2)

    # --- plot nodes ---
    for node_id, x, z in nodes:
        if z == 0:
            ax.plot(x, z, 'ks', markersize=8)  # ground-level node (column base)
        else:
...
def plot_grid_elevation_view_YZ(nodes, elements, ny, nz):
    ...
    # --- plot elements first (so nodes appear on top) ---
    for eid, n1, n2, etype in elements:
        y1, z1 = nodes[n1][1], nodes[n1][2]
        y2, z2 = nodes[n2][1], nodes[n2][2]

        if etype == "column":
            ax.plot([y1, y2], [z1, z2], color="yellow", linewidth=2)
        elif etype == "beam":
            ax.plot([y1, y2], [z1, z2], color="green", linewidth=2)

    # --- plot nodes ---
    for node_id, y, z in nodes:
        if z == 0:
            ax.plot(y, z, 'ks', markersize=8)  # ground-level node (column base)
        else:
            ax.plot(y, z, 'ko', markersize=6)  # upper node

        ax.text(y + 0.05, z + 0.05, str(node_id), fontsize=10)
